class Node:
    ...
    Models two attributes: data and the reference/link to the next node in the list.
    """

    def __init__(self, data, prev_node = None, next_node = None):
        self.data = data
        self.prev_node = prev_node # for doubly linked list only
...
class SinglyLinkedList:
    ...
    def __init__(self):
        # ensures every list is empty upon creation
        self.head = None

    def is_empty(self):
        # method to check that list is, indeed, empty (will return boolean value)
    ...
    def size(self):
        ...
        Takes O(n) time
        """
        # runs in linear time (as opposed to constant time, like a normal list)
        # visits each node and calls next until we reach the tail node
        current = self.head
        count = 0
    ...
    def insert(self, data, index):
        """
        Inserts a ned Node containing data at the next position
